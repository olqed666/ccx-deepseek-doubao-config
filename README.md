...
**重要提醒：使用本配置时，若不选择模型版本为5.5（请勿选择5.6 Sol版本），会出现工具使用泄露问题，请务必注意！**
...
| easoningParamStyle | DeepSeek | 推理参数格式 "reasoning" |
| easoningMapping | DeepSeek | gpt → max 深度思考 |
...
`powershell
copy config.example.json C:\Users\20997\AppData\Roaming\ccx-desktop\.config\config.json
`
...
| 字段 | 值 | 说明 |
|---|---|---|
| aseUrl | https://api.deepseek.com | 直连 DeepSeek API |
| modelMapping | gpt→v4-pro, mini→v4-flash | 模型映射 |
| easoningMapping | {"gpt": "max"} | 深度思考 |
| easoningParamStyle | "reasoning" | 推理参数格式 |
| codexNativeToolPassthrough | 	rue | 透传 Codex 原生工具 |
| codexToolCompat | alse | 不做 tool 格式转换 |
| 
ormalizeNonstandardChatRoles | 	rue | developer → system |
| 
oVision | 	rue | 跳过图片请求 |
...
| 字段 | 值 | 说明 |
|---|---|---|
| aseUrl | https://ark.cn-beijing.volces.com/api/v3 | 火山引擎 ARK |
